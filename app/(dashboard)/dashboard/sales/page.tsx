...
// cache instead of re-running ~20 parallel queries on every page load.

const getSalesDashboardData = unstable_cache(
  async (
    userId: string,
    role: string,
    ...
    return {
      ...
      insightData: {
        ...
        warmLeads,
        hotLeads,
        topHotSource: topHotSource?.lead_source
          ? { lead_source: topHotSource.lead_source, count: topHotSource._count.id }
          : null,
      },
    };
  },
...
export default async function SalesDashboardPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  // ── Smart insights ─────────────────────────────────────────────────────
  const ins = data.insightData;
  const insights: string[] = [];
  if (ins.pendingFirstAction > 0)
    insights.push(`${ins.pendingFirstAction} lead${ins.pendingFirstAction > 1 ? "s" : ""} are waiting for first contact.`);
  if (ins.topHotSource && ins.topHotSource.count > 0)
    insights.push(`"${ins.topHotSource.lead_source}" brought ${ins.topHotSource.count} hot lead${ins.topHotSource.count > 1 ? "s" : ""} ${rangeLabel.toLowerCase()}.`);
  if (ins.overdueFollowUps > 0)
    insights.push(`${ins.overdueFollowUps} overdue follow-up${ins.overdueFollowUps > 1 ? "s" : ""} need immediate attention.`);
  if (ins.staleLeads > 0)
    insights.push(`${ins.staleLeads} active lead${ins.staleLeads > 1 ? "s" : ""} haven't had any update in ${staleDays}+ days.`);
  if (ins.notActioned > 0 && ins.leadsReceived > 0 && ins.responseRate < 50)
    insights.push(`Response rate is ${ins.responseRate}% — ${ins.notActioned} lead${ins.notActioned > 1 ? "s" : ""} received ${rangeLabel.toLowerCase()} haven't been worked on.`);
  if (ins.warmLeads > ins.hotLeads && ins.warmLeads > 5)
    insights.push(`${ins.warmLeads} warm leads are ready to be upgraded to hot.`);
  ...
  return (
    <div className="p-3 sm:p-6 space-y-6">
      ...
      <SalesDashboardClient
        ...
        salesOwnerStats={data.salesOwnerStats}
        leadsPerOpportunity={data.leadsPerOpportunity}
