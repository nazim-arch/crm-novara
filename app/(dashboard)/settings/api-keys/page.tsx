...
// App-wide API Keys settings. The AI keys (Claude / OpenAI) configured here power
// every AI feature across DealStack — IntentRadar insights, scrapers, Pipeline Digest,
// and any future AI functionality. All keys are stored encrypted (see /api/intentradar/settings).
'use client';
...
const ACTOR_FIELDS: ApiKeyField[] = [
  {
    ...
    category: 'actors',
    placeholder: 'Leave empty to use default (apify~instagram-scraper)',
    required: false,
    note: 'Leave blank to use the built-in scraper. Only set this if you have a dedicated comment actor ID from apify.com/store. Format must use ~ not / (e.g. username~actor-name). Clear this field and save if you are seeing "Actor was not found" errors.',
  },
];
...
const API_KEY_GROUPS: { group: string; desc: string; fields: ApiKeyField[] }[] = [
  {
    group: 'AI & NLP',
    desc: 'Powers every AI feature across DealStack — IntentRadar insights, lead scoring, and Pipeline Digest',
    fields: [
      { key: 'api_key_claude', label: 'Anthropic Claude API Key', category: 'api_keys', placeholder: 'sk-ant-...', required: false, note: 'Used for all Claude-powered AI across DealStack.' },
      { key: 'api_key_openai', label: 'OpenAI (ChatGPT) API Key', category: 'api_keys', placeholder: 'sk-...', required: false, note: 'Used for all ChatGPT-powered AI across DealStack.' },
    ],
  },
  {
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #4338ca, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: 18 }}>🔑</div>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, margin: 0, color: '#1c1917' }}>API Keys</h1>
            <p style={{ fontSize: 13, color: '#78716c', margin: 0 }}>Keys used across all AI features and IntentRadar in DealStack</p>
          </div>
        </div>
